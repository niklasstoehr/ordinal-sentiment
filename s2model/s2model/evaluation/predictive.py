...
def impute_infer_Z(impute_site = [], data = {}, posterior_samples = {}, m = None, infer = 1):
    ...
    ## infer P(Z | imputed_data, theta)______
...
def make_prediction(posterior_samples, m, data, infer= 1, sites={"SW": "mean", "MP": "mode", "SC": "mean", "VA": "mean", "HL": "mode", "GI": "mode"}):

    pooled_sites_dict = dict()
    sites_loglik_dict = dict()

    for site in sites.keys():
        if site in data["mask"].keys():
            ...
            pooled_imputed_sites, pooled_imputed_sites_conf = trace_handlers.pool_posterior(imputed_site, pool_type=sites)

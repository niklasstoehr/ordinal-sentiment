...
def pool_posterior(sample_dict, pool_type = {"SW": "mean", "MP": "mode", "SC": "mean", "VA": "mean", "HL": "mode", "GI": "mode"}):
    ...
    for k in sample_dict.keys():
        ...
        if len(k_tensor.shape) == 1:
            ...
            if is_decimal > 0 or pool_type == "mean":
...
def sites_to_tensor(trace, sites=["VA", "Z"]):
    sites_log = dict()
    sites_data = dict()

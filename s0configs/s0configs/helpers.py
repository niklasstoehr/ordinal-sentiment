...
def store_model(m = None, posterior_params = None):
    ...
    path_name = config.get_path("models") / Path(str(m.model_name) + "_nc_" + str(m.n_c) + ".pkl")
...
def load_model(file_name="model_full_nc_4"):
...
def store_df(df=None, path_name = "proc_sentiments", file_name = "df"):

    config = configs.ConfigBase()
    path_name = config.get_path(path_name) / Path(file_name + ".csv")
    df.to_csv(path_name, index=False)
    print(f"stored annotations to {path_name}")


def load_df(path_name = "proc_sentiments", file_name="min_nan_count_2"):

    config = configs.ConfigBase()
    path_name = config.get_path(path_name) / Path(str(file_name) + ".csv")
    df = pd.read_csv(path_name)
    print(f"loaded annotations from {path_name}")
    return df



def pickle_data(data=None, path_name="proc_sentiments", file_name="min_nan_count_2"):

    config = configs.ConfigBase()
    path_name = config.get_path(path_name) / Path(file_name + ".pkl")
    with open(path_name, 'wb') as f:
        pickle.dump(data, f)
    print(f"pickled annotations to {path_name}")



def load_pickle(path_name = "proc_sentiments", file_name="min_nan_count_2"):

    config = configs.ConfigBase()
    path_name = config.get_path(path_name) / Path(str(file_name) + ".pkl")
    with open(path_name, 'rb') as f:
        data = pickle.load(f)
    print(f"loaded annotations from {path_name}")
    return data
